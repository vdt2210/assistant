...
def assistantinfo(user):
	...
	elif user == "what your name stand for":
...
def assistant():
	...
	elif os.path.exists('./username.ini') == False or os.stat('./username.ini').st_size == 0:
		speak("Hi there! My name is FVA. What your name?")
		...
		while True:
			...
			if "assistant" in user:
				if user == "assistant":
					speak("Yes?")
					user = userspeak()
				elif "assistant" in user:
					user = user.replace("assistant ", "")
				if not user or user == "":
					pass
					...
					userinfo()
				elif "what's your name" in user or "what your name stand for" in user:
					assistantinfo(user)
